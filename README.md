# Bridgelabz Java

Java assignments and practice problems completed as part of the BridgeLabz training program. Assignments are taken from Google Classroom (GCR).

## Repository Structure

- `main`: README and daily task updates
- `develop`: empty Java project skeleton
- `feature/*`: week-wise topic branches containing the solutions

---

## Daily Task Update

### Day 04 - 23 Sep 2026

**What I have done**

- Completed on Java Arrays and pushed to GitHub

**What I will do**

- Will move on to Assignment problems from Java Methords

### Day 03 - 22 Sep 2026

**What I have done**

- Worked on Java Arrays
- Completed one question in level 1 and 4 in level 2

**What I will do**

- Complete the remaining Assignment problems

### Day 03 - 21 Sep 2026

**What I have done**

- Worked on Java programming elements remaining questions
- Solved 28 Questions from Java Control Flow Assignment
- (A few problems are still left from level 2 and full level 3 in Java Control Flow, which will be completed by the mid of this week)

**What I will do**

- Complete the remaining Assignment problems and hopefully from new assignments

**Issues faced**

- Comparatively slow in finding real use cases for switch function (as it is a new concept for me)

---

### Day 02 - 19 Sep 2026

**What I have done**

- Understood the concepts of loops in more detail
- Implemented some of the types in IntelliJ IDEA
- Completed 15 questions of different level Java Programming Elements Assignment

**What I will do**

- Complete the remaining Assignment problems
- Explore some useful libraries and functions that can help further in coding

**Issues faced**

- Got really confused between understanding how I can apply Math.random for different ranges
- Understood the issue and also learnt the Random library

---

### Day 01 - 18 Sep 2026

**What I have done**

- Set up the repository with main, develop and feature branches
- Solved 5 problems on variables and data types (`feature/coreProgramming` > `javaProgrammingElement`)

**What I will do**

- Complete the remaining operator problems
- Start the `javaControlFlow` package (if-else and switch)

**Issues faced**

- Confused between different types of branch commands
'@ | Set-Content README.md